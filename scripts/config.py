...
# Locations searched when READING: the canonical path first, then the legacy
# repo-root location so a Parquet file from an earlier run still resolves.
_PARQUET_CANDIDATES = (
    PARQUET_WRITE_PATH,
    ROOT / PARQUET_NAME,
)
...
def _resolve_parquet() -> Path:
    ...
    Returning the canonical path when nothing exists keeps the eventual error
    pointing at the place the file SHOULD be, which is more useful than a bare
    relative filename.
    """
